...
import {NeonResponse, NeonScrollPayResponse} from './response';
...
export class NeonGenesisService {
  ...
  public getAllPaidItems(address: string): Observable<NeonResponse<any>> {
    const params = {
      v: 3,
      q: {
        find: {
          'out.s1': Const.SPLITPAY_PROTOCOL,
          'in.e.a': address
        }
      },
      r: {
        f: '[.[] | { order: .i, txid: .tx.h, block: .blk, pushdata: ' +
          '{ b0: "OP_RETURN", itemid: .out[0].s2, from: .out[0].s3, until: .out[0].s4} }]'
      }
    };
    return this.http.get<any>(NEON_ENDPOINT + btoa(JSON.stringify(params)), httpOptions);
  }
